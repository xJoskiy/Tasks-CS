...
namespace Clock
{
    ...
    public class Clock
    {
        public static void Main(string[] args)
        {
            while (true)
            {
                ...
                do
                {
                    ...
                    Console.WriteLine("Your input is not an integer number in the interval [0;180], try again");
        ...
        static public string ComputeTime(float setDegrees, int hours, int minutes)
        {
            ...
            if (setDegrees >= curDegree && curDegree <= 180)
                minutes += (int)Math.Ceiling((setDegrees - curDegree) / 5.5);

            else if (setDegrees < curDegree && curDegree <= 180)
                minutes += (int)Math.Ceiling((360 - curDegree - setDegrees) / 5.5);

            else if (setDegrees <= 360 - curDegree && curDegree > 180)
                minutes += (int)Math.Ceiling((360 - curDegree - setDegrees) / 5.5);
        ...
        static public float ComputeDegree(int hours, int minutes) // Угол между часовой и минутной считая по направлению часовой стрелки
        {
